...
export class OrderService {
  ...
  async findAllOrdersForAdmin(adminId: number, userId?: number) {
  // Verify super admin
  const isSuperAdmin = await this.prisma.admin.findUnique({
    where: { id: adminId },
  });
  if (!isSuperAdmin) {
    throw new ForbiddenException('User is not a super admin');
  }
  ...
  // Generate signed URLs for product images and user profile
  const enrichedOrders = await Promise.all(
    orders.map(async (order) => {
      const enrichedOrderItems = await Promise.all(
        order.order_items.map(async (item) => {
          const imagesWithUrls = item.product
            ? await Promise.all(
                item.product.product_images.map(async (image) => ({
                  ...image,
                  image_url: await this.s3Service.get_image_url(image.image_url),
      ...
      const userProfileUrl = order.users?.profile
        ? await this.s3Service.get_image_url(order.users.profile)
        : null;
      return {
        ...order,
        shipping_rate: parseFloat(order.shipping_rate),
...
async updateOrderPaymentStatus(orderId: number, adminId: number) {
  // Verify super admin
  const isSuperAdmin = await this.prisma.admin.findUnique({
    where: { id: adminId },
  });
  ...
  // Toggle payment_status
  const newStatus = order.payment_status === 'pending' ? 'complete' : 'pending';
