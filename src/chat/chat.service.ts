...
async getCommunityChatMessages(
  ...
  try {
    ...
    // Skip banned user check for super admins
    if (!isSuperAdmin && chat.banned_users.some((user) => user.id === validUserId)) {
      throw new BadRequestException(
        'User is banned from this community chat',
      );
    }
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          wallpaper: wallpaperKey
            ? await this.getSignedImageUrl(wallpaperKey)
            : null,
  ...
  async deleteCommunityChat(communityChatId: number, requesterId: number) {
    try {
      ...
      if (
        ...
        throw new BadRequestException(
          'User is not authorized to delete this community chat',
        );
      }
      ...
      throw new BadRequestException(
        'Failed to delete community chat: ' + error.message,
      );
    }
  }
  async getCommunityChat(communityChatId: number, userId: number) {
    try {
      ...
      const bannedUsers = await Promise.all(
        chat.banned_users.map(async (user) => ({
          ...
          profile_picture: user.profile
            ? await this.getSignedImageUrl(user.profile)
            : null,
        })),
      );
      ...
      throw new BadRequestException(
        'Failed to retrieve community chat: ' + error.message,
      );
    }
  }
  async getTopCommunityChatsByMessages(limit: number, userId: number) {
    try {
          ...
          return {
            ...
            latest_message: latestMessage
              ? {
                  ...
                  users: {
                    username:
                      latestMessage.users?.username ||
                      latestMessage.admin?.name ||
                      'Unknown',
      ...
      throw new BadRequestException(
        'Failed to retrieve top community chats: ' + error.message,
      );
    }
  }
  ...
  //     if (!chat) {
  //       throw new BadRequestException(
  //         'Community chat not found or user is not authorized',
  //       );
  //     }
  ...
  //     throw new BadRequestException(
  //       'Failed to retrieve banned users: ' + error.message,
  //     );
  //   }
  // }
  async getBannedUsers(communityChatId: number, userId: number) {
  try {
    ...
    // Fetch the chat with banned users
    const chat = await this.prismaService.community_chat.findFirst({
      where: isSuperAdmin
        ? { id: validChatId }
        : {
