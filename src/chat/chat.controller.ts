...
import { Controller, Get, Post, Body, Query, BadRequestException, UseGuards, Req,Param } from '@nestjs/common';
...
export class ChatController {
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized - Invalid or missing token' })
  async getCommunityChatMessages(@Param('communityChatId') communityChatId: string) {
    try {
      const parsedChatId = parseInt(communityChatId);
      if (isNaN(parsedChatId)) {
        throw new BadRequestException('Invalid community chat ID');
      }
      return await this.chatService.getCommunityChatMessages(parsedChatId);
    } catch (error) {
 ...
 @Get('/community/list')
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized - Invalid or missing token' })
  async getCommunityChats(@Query('limit') limit: string) {
    try {
      const parsedLimit = parseInt(limit);
      if (isNaN(parsedLimit) || parsedLimit <= 0) {
        throw new BadRequestException('Invalid limit parameter');
      }
      return await this.chatService.getCommunityChats(parsedLimit);
    } catch (error) {
  ...
  @ApiOperation({ summary: 'Create a new community chat with a name and optional description' })
  @ApiBody({
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized - Invalid or missing token' })
  async createCommunityChat(@Body() body: { name: string; description?: string }, @Req() request: Request & { user: { id: number } }) {
    try {
      return await this.chatService.createCommunityChat(body.name, body.description, request.user.id);
    } catch (error) {
